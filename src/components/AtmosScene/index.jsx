...
export default function AtmosScene() {
  ...
  // Ref mirror for use inside rAF without stale closure
  const experienceDoneRef = useRef(false);
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      // If experience is done and user is scrolling down, let page scroll naturally
      if (experienceDoneRef.current && e.deltaY > 0) return;

      // If at top and scrolling up, let browser handle it naturally
      if (targetScroll.current <= 0 && e.deltaY < 0) return;
    ...
    const smoothScroll = () => {
      ...
      // Mark experience as done when near the end
      const done = currentScroll.current >= 0.98;
      if (done !== experienceDoneRef.current) {
        experienceDoneRef.current = done;
    ...
    const handleTouchMove = (e) => {
      // If experience is done and scrolling down, let page handle it
      if (experienceDoneRef.current) return;
